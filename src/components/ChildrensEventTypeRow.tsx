...
function ChildrensEventTypeRow({
	eventType,
	removeEventType,
	updateEventType,
	setSnackBar
}: {
	eventType: childrensEventTypeType;
	removeEventType: () => void;
	updateEventType: (eventType: childrensEventTypeType) => void;
	...
	async function deleteEventType() {
		...
		const { error } = await supabase.from("childrenEventTypes").delete().eq("id", eventType.id);
		if (error) {
			console.error(error);
	...
	async function saveEventType() {
		...
		if (error || !data || data.length == 0) {
			...
			updateEventType(data[0] as childrensEventTypeType);
			setEditing(false);
		}
...
export default ChildrensEventTypeRow;
